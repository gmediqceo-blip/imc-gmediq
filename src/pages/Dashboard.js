...
const NAV_ITEMS = [
  ...
  { key: 'caja', label: 'Caja', icon: 'wallet', soloCaja: true },
];
...
export default function Dashboard({ session }) {
  ...
  if (cuenta === 'caja' && miembroCaja) {
    return (
      <div className="v2" style={{ minHeight: '100vh', background: 'var(--canvas)', color: 'var(--ink)', fontFamily: FUENTE }}>
        <div style={{ background: 'var(--ink)', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ color: '#fff', fontWeight: 600, fontSize: 15, margin: 0 }}>Caja</p>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 11, margin: 0 }}>{miembroCaja.nombre}</p>
          </div>
          <button onClick={handleLogout} style={{ ...btnFantasma, background: 'rgba(255,255,255,.12)', color: '#fff', border: '1px solid rgba(255,255,255,.18)' }}>
            <Icon name="log-out" size={15} color="#fff" /> Salir
          </button>
        </div>
  ...
  const navItems = NAV_ITEMS.filter(it =>
    (!it.soloAdmin || usuario?.rol === 'admin') &&
    (!it.soloCaja || miembroCaja));
